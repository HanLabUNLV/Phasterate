  ...
  for (rcat = 0; rcat < mod->nratecats; rcat++) {
    for (nodeidx = 0; nodeidx < lst_size(traversal); nodeidx++) {
      ...
      if (n->lchild == NULL) {
        ...
        char c = msaFlag ?
          ss_get_char_tuple(msa, tupleidx, mod->msa_seq_idx[n->id], 0) :
          msa_get_char(msa, mod->msa_seq_idx[n->id], tupleidx);
...
double col_compute_log_likelihood(TreeModel *mod, MSA *msa, int tupleidx,
  ...
  if(mod->subst_mod == F84E){
    if(mod->originalF84E == 1)
      results = log(singleSiteLikelihoodRivas(mod, msa, tupleidx, scratch, flagTrue));
    else
      results = log(singleSiteLikelihoodModified(mod, msa, tupleidx, scratch, flagTrue));
  }
  else
    results = log(col_compute_likelihood(mod, msa, tupleidx, scratch,flagTrue));
...
//==========================================================================================
/*
 * This function performs the original F84 extended prunning algorithm as described by
 * Rivas & Eddy (2008).
 ...
 * Given a TreeModel, an MSA, a column number and allocated table to hold the probabilities.
 * It will return the total likelihood for a tree. The rest of parameters are allocated memory.
 * See @gapAwareLikelihood for allocations. This is function does the bulk of the work.
...
*/
double singleSiteLikelihoodRivas(TreeModel* mod, MSA* msa,int tupleidx, double** likelihoodTable,
        int flag){
  int i;
  int nstates = mod->rate_matrix->size;
  double totalProb;
...
//==========================================================================================
/*
 * This function performs a modified F84 extended prunning algorithm as described in the
 * PhastIndel paper.
 * Extremely similar to double computeTotalTreeLikelihood() from tree_likelihoods.c
 * but only calculates the scores for the given tupleIdx instead of the whole alignment
 * basically does the work of computing for one column.
 ...
 * @param flag: If flag is true is will assume sufficient statistics, else it will not.
*/
double singleSiteLikelihoodModified(TreeModel* mod, MSA* msa,int tupleidx, double** likelihoodTable,
        int flag){
  int i;
  int nstates = mod->rate_matrix->size;
  double totalProb;
